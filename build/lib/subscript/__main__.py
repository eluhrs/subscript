...
def load_config(config_path=None):
    ...
    if config_path and os.path.exists(config_path):
        return yaml.safe_load(open(config_path, 'r')), config_path
    
    # 2. CWD config.yml
    cwd_config = os.path.join(os.getcwd(), 'config.yml')
    if os.path.exists(cwd_config):
        return yaml.safe_load(open(cwd_config, 'r')), cwd_config
        
    # 3. Package default config.yml
    package_dir = os.path.dirname(__file__)
    # Go up one level if we are in src/subscript (we are in subscript package)
    # Actually, config.yml is at root of repo, but setup.py puts it in package root?
    # In pyproject.toml we said: subscript = ["config.yml"] and package-dir = {"" = "src"}
    # So config.yml should be next to __init__.py in site-packages/subscript/config.yml
    pkg_config = os.path.join(package_dir, 'config.yml')
    if os.path.exists(pkg_config):
        return yaml.safe_load(open(pkg_config, 'r')), pkg_config
        
    logger.error(f"Config file not found. Checked: {config_path}, {cwd_config}, {pkg_config}")
    sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Subscript HTR pipeline: image segmentation, transcription, and searchable PDF conversion\n\n"
                    ...
                    "Usage: subscript [SEGMENTATION-NICKNAME] [MODEL-NICKNAME] INPUT [OPTIONS]\n\n"
                    "Note: Input files and output directory are relative to the current working directory.",
    ...
    parser.add_argument("--config", metavar="", help="Path to alternate config file (default: ./config.yml or package default)")
    ...
    logger.info(f"Using config: {loaded_config_path}")
